 ...
 */
import { groupByCluster } from './resolver.js';
/**
 * Build the dependency graph for environment variables
 */
export function buildEnvGraph(resolved) {
    const nodes = [];
    const edges = [];
 ...
 */
export function getClusterInfo(graph) {
    const clusterInfo = [];
    for (const [clusterName, varNames] of graph.clusters) {
        ...
        for (const node of clusterNodes) {
            ...
            for (const neighbor of neighbors) {
                const edge = graph.edges.find(e => (e.from === node.name && e.to === neighbor) ||
 ...
 */
export function findStrongConnections(graph) {
