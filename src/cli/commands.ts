...
export const commands: Command[] = [
    {
        ...
        handler: async (args, projectPath) => {
            ...
            try {
                const filter = args[0] as 'all' | 'missing' | 'unused' | 'risky' | 'undocumented' | undefined;
                const result = await getEnvMap({ projectPath, filter });

                spinner.stop();
                renderSummary(result as any);
                renderVariables(result as any);
            } catch (error) {
                spinner.stop();
    ...
    },
    {
        ...
        handler: async (args, projectPath) => {
            ...
            try {
                const minRisk = args[0] as 'info' | 'low' | 'medium' | 'high' | 'critical' | undefined;
                const result = await getEnvRisk({ projectPath, minRisk });

                spinner.stop();
                renderRiskAnalysis(result as any);
            } catch (error) {
                spinner.stop();
                renderError(error instanceof Error ? error.message : String(error));
            }
    ...
    },
    {
        ...
        handler: async (args, projectPath) => {
            ...
            try {
                ...
                spinner.stop();
                renderValidation(result as any);
            } catch (error) {
                spinner.stop();
                renderError(error instanceof Error ? error.message : String(error));
            }
