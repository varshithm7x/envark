...
import { type EnvVariable, type ResolvedEnvMap, findSimilarNames, findConflictingDefinitions } from './resolver.js';
 ...
 */
export function analyzeEnvVariables(resolved: ResolvedEnvMap): AnalysisResult {
    const duplicateGroups = findConflictingDefinitions(resolved);
    const similarNameGroups = findSimilarNames(resolved);

    const analyzedVariables: AnalyzedVariable[] = [];
    const summary = {
    ...
    for (const variable of resolved.variables.values()) {
        ...
        const analyzed: AnalyzedVariable = {
            ...variable,
            riskLevel,
            issues,
            recommendations,
    ...
    analyzedVariables.sort((a, b) => {
        const riskOrder: Record<RiskLevel, number> = { critical: 5, high: 4, medium: 3, low: 2, info: 1 };
 ...
 */
export function generateRiskReportSummary(result: AnalysisResult): string {
    const lines: string[] = [];
