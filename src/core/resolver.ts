 ...
 */
function isActualEnvFile(relativePath: string): boolean {
    const name = basename(relativePath).toLowerCase();
    return (
        name.startsWith('.env') &&
 ...
 */
export function resolveEnvMap(usages: EnvUsage[]): ResolvedEnvMap {
 ...
 */
export function findSimilarNames(resolved: ResolvedEnvMap): Map<string, string[]> {
...
export function findConflictingDefinitions(
    resolved: ResolvedEnvMap
): Map<string, Array<{ file: string; value: string }>> {
    const conflicts = new Map<string, Array<{ file: string; value: string }>>();

    for (const [name, variable] of resolved.variables) {
        if (variable.defaultValues.length > 1) {
            // Multiple different values found
            const valuesByFile: Array<{ file: string; value: string }> = [];
 ...
 */
export function groupByCluster(resolved: ResolvedEnvMap): Map<string, EnvVariable[]> {
    const clusters = new Map<string, EnvVariable[]>();

    // Define cluster patterns
    const clusterPatterns: Array<[RegExp, string]> = [
