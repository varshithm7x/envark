...
import { createOllamaProvider, OllamaProvider } from './ollama.js';
...
export class AegisAIAgent {
    ...
    private autoConfigureProvider(): void {
    ...
    configure(config: AIAgentConfig, persist: boolean = true): void {
    ...
    getProviderInfo(): { name: string; model: string; configured: boolean } {
        return {
    ...
    async suggestVariableImprovements(variableName: string, currentValue?: string): Promise<string[]> {
        if (!this.provider) {
    ...
    async generateEnvTemplate(projectType: string, requirements?: string): Promise<string> {
        if (!this.provider) {
...
`;
    ...
    async generateValidationCode(variables: string[], language: string = 'typescript'): Promise<string> {
        if (!this.provider) {
            return '// AI not configured';
        }
    ...
    async explainVariable(variableName: string): Promise<string> {
...
Be concise but comprehensive.`;
    ...
    async assessSecurityRisk(variableName: string, value?: string): Promise<{
...
// Singleton instance
let agentInstance: AegisAIAgent | null = null;

export function getAIAgent(): AegisAIAgent {
    if (!agentInstance) {
...
export function resetAIAgent(): void {
    agentInstance = null;
}
