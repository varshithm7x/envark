 ...
 */
function suggestDescription(name: string, context: string): string {
    // Common patterns
 ...
 */
export async function getUndocumented(input: GetUndocumentedInput): Promise<GetUndocumentedOutput> {
    ...
    for (const [name, variable] of resolved.variables) {
        // Skip if documented
        if (variable.isDocumented) continue;

        // Only include variables actually used in code
        if (!variable.usedInCode) continue;
